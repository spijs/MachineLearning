...
% #4 There need to be enough spaces left not taken by a ship to fulfill the tally
...
% #6 Add a field that was deducted from the hints or other fields.
...
count(b,I),count(t,J),count(l,K),count(r,L) ==> T is I+J, S is K+L, Tot is S+T, Tot > 12 | false.
 ...
 % #8 If all boats are chosen, fill with water.
 nbSub(S), nbBattle(B), nbCruise(C), nbDest(D) , field(R,Co,_,_) ==> S=:=4,D=:=3,C=:=2,B=:=1 | add1(R,Co,water).
 
 % #9 If all subs are chosen, remove circle as a possibility.
 nbSub(I) \ field(R,C,N,L) <=> I =:= 4, select(circle,L,LL), N1 is N-1, N1>0 | field(R,C,N1,LL).
 
 % #10 Edit the amount of chosen boat parts in both the column and row.
 ...
 % #11 row or column are filled -> fill with water.
 ...
 % #12 Limit possibilities near a water field.
 ...
 % #13 Edit the amount of chosen water parts in both the column and row.
 ...
 % #14 Remove column and row constraint if they are completely filled.
 column(_,N,N,_,_) <=> true.
 row(_,N,N,_,_) <=> true.
 
 % #15 Limit possibilities of edges 
 ...
 % #16 Boats can not touch diagonally
 ...
 % #17 Limit possibilities next to certain boat parts.
...
% #23 remove unnecessary constraints when finished.
