...
number_row(_,B,C),number_row(_,B,C) <=> false.
%add
...
% same block    
same_block @ number_row(A,R,C) \ number_row(A,R2,N,L) <=> select(V,L,LL), same_block(R,C,R2,V) |
    N1 is N-1, N1>0, number_row(A,R2,N1,LL).

% One value left
number_row(A,B,1,[V]) <=> number_row(A,B,V).
